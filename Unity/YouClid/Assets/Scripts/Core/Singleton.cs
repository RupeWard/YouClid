        ...
        get
        {
            if (SingletonHelper<GameObjectType>.ShouldAssignInstance(ref instance, ref blockInstanceFetch))
            {
				...
				if (instances.Length == 1)
				{
					...
					instances[0].gameObject.name = instanceName; 
					#if DEBUG_SINGLETONS
				...
				{
					Debug.LogError("Found "+instances.Length+" in looking for scene singleton instance of type " + typeof(GameObjectType).ToString());
				}
	...
	where GameObjectType : MonoBehaviour
{
	...
	void OnDestroy()
	{
		...
		if (instance != null)
		{
			...
			{
				#if DEBUG_SINGLETONS
				Debug.Log("Destroying the instance of  " + typeof(GameObjectType).ToString());
				#endif //DEBUG_SINGLETONS
				SingletonHelper<GameObjectType>.HandleOnDestroy(ref instance, ref blockInstanceFetch);
				PostOnDestroy();
			}
		...
		get
		{
			if (SingletonHelper<GameObjectType>.ShouldAssignInstance(ref instance, ref blockInstanceFetch))
			{
				...
				if (gameObj != null)
				{
					...
					#if DEBUG_SINGLETONS
					Debug.Log("Created a singleton with an scene lifetime of type " + typeof(GameObjectType).ToString());
					#endif //DEBUG_SINGLETONS
